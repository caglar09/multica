...
func (e *BrainRuntimeExecutor) Execute(ctx context.Context, cfg projectorchestration.BrainConfig, evidence json.RawMessage) (BrainRuntimeResult, error) {
	...
	if err != nil {
		return BrainRuntimeResult{}, fmt.Errorf("create brain session: %w", err)
	}
	...
	if err != nil {
		return BrainRuntimeResult{}, fmt.Errorf("enqueue brain learning task: %w", err)
	}
...
func (e *BrainRuntimeExecutor) ensureCarrier(ctx context.Context, cfg projectorchestration.BrainConfig) (db.Agent, db.AgentRuntime, error) {
	...
	if _, err := tx.Exec(ctx, "SELECT pg_advisory_xact_lock(hashtextextended($1,0))",
		"autonomous-project-brain:"+util.UUIDToString(cfg.ProjectID)); err != nil {
		return db.Agent{}, db.AgentRuntime{}, err
	}
	...
	if err != nil {
		return db.Agent{}, db.AgentRuntime{}, fmt.Errorf("load Mika for brain owner: %w", err)
	}
	mika, err = q.GetAgentForUpdate(ctx, mika.ID)
	if err != nil {
		return db.Agent{}, db.AgentRuntime{}, fmt.Errorf("lock Mika profile for brain: %w", err)
	}
	...
	if cfg.RuntimeMode == "custom" {
		if !cfg.RuntimeID.Valid {
			return db.Agent{}, db.AgentRuntime{}, errors.New("custom brain runtime is missing")
		}
		...
		`, runtimeID, cfg.WorkspaceID).Scan(&mode); err != nil {
			if errors.Is(err, pgx.ErrNoRows) {
				return db.Agent{}, db.AgentRuntime{}, errors.New("configured brain runtime is unavailable")
			}
			return db.Agent{}, db.AgentRuntime{}, err
		}
		runtimeMode = mode
		...
		if cfg.Model != "" {
			model = pgtype.Text{String: cfg.Model, Valid: true}
		}
		if cfg.ThinkingLevel != "" {
			thinking = pgtype.Text{String: cfg.ThinkingLevel, Valid: true}
		}
		if cfg.ServiceTier != "" {
			tier = pgtype.Text{String: cfg.ServiceTier, Valid: true}
		}
	}
	if !runtimeID.Valid {
		return db.Agent{}, db.AgentRuntime{}, errors.New("brain runtime is not configured")
	}
	...
	if err != nil {
		return db.Agent{}, db.AgentRuntime{}, fmt.Errorf("load brain runtime: %w", err)
	}
	if runtime.Status != "online" {
		return db.Agent{}, db.AgentRuntime{}, fmt.Errorf("configured brain runtime %q is %s", runtime.Name, runtime.Status)
	}
	...
	if err != nil {
		return db.Agent{}, db.AgentRuntime{}, fmt.Errorf("ensure brain carrier: %w", err)
	}
...
func (e *BrainRuntimeExecutor) wait(ctx context.Context, taskID pgtype.UUID) (string, error) {
	ticker := time.NewTicker(brainPollPeriod)
	defer ticker.Stop()
	for {
		...
		case "completed":
			...
			if strings.TrimSpace(result.Output) == "" {
				return "", errors.New("brain runtime returned empty output")
			}
			return result.Output, nil
		case "failed":
			if task.Error.Valid {
				return "", errors.New(strings.TrimSpace(task.Error.String))
			}
			return "", errors.New("brain runtime task failed")
		case "cancelled", "canceled":
			return "", errors.New("brain runtime task cancelled")
		}
		select {
		case <-ctx.Done():
			c, cancel := context.WithTimeout(context.Background(), 5*time.Second)
			_, _ = e.taskSvc.CancelTask(c, taskID)
			cancel()
			return "", ctx.Err()
		case <-ticker.C:
		}
