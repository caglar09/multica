...
Repository storage and task workspaces are deliberately separate. The daemon keeps one canonical bare repository cache per workspace and repository URL under its workspace root; each task gets its own branch and worktree (or isolated checkout) under its task workspace. Seeing two task `workdir` paths therefore does not mean two canonical repositories were configured. The checkout response exposes both paths as `path` and `canonical_repo_path` so operators can verify this directly.
